...
function toggleFavorite(linkFrom, spawnID) {
	$.post(BASE_SCRIPT_URL + "setFavorite.py",{ itemID: spawnID, favType: 1 },
		function(data) {
			if (data.substr(0,5) == "Error") {
				alert(data);
			} else {
...
function updateFavoriteGroup(spawnID, groupName) {
	$.post(BASE_SCRIPT_URL + "setFavorite.py",{ itemID: spawnID, favType: 1, favGroup: groupName, op: 2 },
		function(data) {
			if (data.substr(0,5) == "Error") {
				alert(data);
			} else {
...
/*   Recipe Functions */
function newRecipe(schematicID, recipeName) {
	$('#busyImgAdd').css('display','block');
	$.post(BASE_SCRIPT_URL + "postRecipe.py", { schematic: schematicID, recipeName: recipeName },
		function(data) {
			var result = $(data).find('resultText').eq(0).text();
...
function saveSuggestedRecipe(schematicID, recipeName, ingredients) {
	$('#busyImgSave').css('display','block');
	$.post(BASE_SCRIPT_URL + 'postRecipe.py', { schematic: schematicID, recipeName: recipeName, ingredients: ingredients },
		function(data) {
			var result = $(data).find('resultText').eq(0).text();
			$('#busyImgSave').css('display','none');
			refreshRecipes();
