...
if (logged_state > 0):
	...
	if (cursor):
		...
		if (spawnName == ''):
			cursor.execute('SELECT spawnName FROM tResources WHERE spawnID=' + spawnID + ';')
			row = cursor.fetchone()
		...
		if (spawnID != '' and galaxy != ''):
			if (dbShared.galaxyState(galaxy) == 1):
				...
				if (availability == "0"):
					...
					dbShared.logEvent("INSERT INTO tResourceEvents (spawnID, userID, eventTime, eventType, planetID) VALUES (" + str(spawnID) + ",'" + currentUser + "',NOW(),'p'," + str(planets) + ");",'p',currentUser,galaxy, str(spawnID))
