	...
	if (cursor):
		if uid != '':
			...
			if period == 'current':
				...
				sqlStr = sqlStr + '(SELECT userID, \'Adds\' AS action, Count(*) AS numActions FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID WHERE galaxy=' + galaxy + ' AND unavailable IS NULL AND eventType=\'a\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ' UNION (SELECT userID, \'Planet Add\', Count(*) FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID WHERE galaxy=' + galaxy + ' AND unavailable IS NULL AND eventType=\'p\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ' UNION (SELECT userID, \'Verified\', Count(*) FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID WHERE galaxy=' + galaxy + ' AND unavailable IS NULL AND eventType=\'v\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ' UNION (SELECT userID, \'Waypoints\', Count(*) FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID WHERE galaxy=' + galaxy + ' AND unavailable IS NULL AND eventType=\'w\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ') eventData INNER JOIN tUsers ON eventData.userID = tUsers.userID;'
			elif period == 'category':
				sqlStr = 'SELECT eventData.userID, action, numActions, pictureName FROM ('
				sqlStr = sqlStr + '(SELECT userID, \'Creature\' AS action, Count(*) AS numActions FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID INNER JOIN tResourceTypeGroup ON tResources.resourceType = tResourceTypeGroup.resourceType WHERE galaxy=' + galaxy + ' AND resourceGroup = \'creature_resources\' AND unavailable IS NULL AND eventType=\'a\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ' UNION (SELECT userID, \'Flora\', Count(*) FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID INNER JOIN tResourceTypeGroup ON tResources.resourceType = tResourceTypeGroup.resourceType WHERE galaxy=' + galaxy + ' AND resourceGroup = \'flora_resources\' AND unavailable IS NULL AND eventType=\'a\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ' UNION (SELECT userID, \'Inorganic\', Count(*) FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID INNER JOIN tResourceTypeGroup ON tResources.resourceType = tResourceTypeGroup.resourceType WHERE galaxy=' + galaxy + ' AND resourceGroup = \'inorganic\' AND unavailable IS NULL AND eventType=\'a\' GROUP BY userID ORDER BY Count(*) DESC LIMIT 1)'
				sqlStr = sqlStr + ') eventData INNER JOIN tUsers ON eventData.userID = tUsers.userID;'
			else:
