...
def getResourceSQL(maxCheckStr, favCols, joinStr, galaxy, resGroup, obyStr, obyStr2, mine):
	...
	if unavailable == '1' or mine != '':
		sqlStr1 += ' WHERE galaxy=' + galaxy
	else:
		sqlStr1 += ' WHERE galaxy=' + galaxy + ' AND unavailable IS NULL'
	if mine != '':
...
if (errstr != ''):
	...
	if (ingCursor):
		...
		while (ingRow != None):
			if (resGroup != ingRow[0]):
				resGroup = ingRow[0]
				print '<h3 class="standOut">' + ingRow[1] + '</h3>'
			...
			if (obyStr != ''):
				...
				if compare == 'undefined' or logged_state == 0:
					...
					print getResourceData(conn, resSQL, logged_state, galaxyState, resourceFormat)
				else:
