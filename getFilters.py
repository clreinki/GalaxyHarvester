...
if (errstr == ""):
	...
	if (logged_state > 0):
		...
		cursor = conn.cursor()
		cursor.execute("SELECT rowOrder, fltType, fltValue, alertTypes, CRmin, CDmin, DRmin, FLmin, HRmin, MAmin, PEmin, OQmin, SRmin, UTmin, ERmin FROM tFilters WHERE galaxy=" + str(galaxy) + " AND userID='" + currentUser + "' ORDER BY rowOrder;")
		row = cursor.fetchone()
		while row != None:
			fltType = row[1]
			fltValue = row[2]
