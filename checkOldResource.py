...
if (cursor):
	cursor.execute('SELECT spawnID, spawnName, resourceType, entered FROM tResources WHERE galaxy=' + galaxy + ' AND spawnName != \'' + spawnName + '\' AND unavailable IS NULL AND resourceType=(SELECT resourceType FROM tResources WHERE spawnName=\'' + spawnName + '\') AND resourceType IN (SELECT resourceType FROM tResourceTypeGroup WHERE resourceGroup IN (\'flora_resources\',\'creature_resources\') GROUP BY resourceType);')
	row = cursor.fetchone()
